...
module.exports = function(RED) {
	...
	const sysmodule = syslib.LoadModule("rpi_mcp23008.node");

	const MODE_INPUT       = "0";
	const MODE_INPUTPULLUP = "1";
	const MODE_OUTPUT      = "2";

	RED.nodes.registerType("mcp23008", function (n) {
		var node = this;
		...
		if (!node.iserror) {
			if (sysmodule.inuse(node.devadr))
				node.iserror = syslib.outError(node, "in use", "address in use");
			else if (!sysmodule.initDIO(node.devadr, parseInt(n.mode)))
